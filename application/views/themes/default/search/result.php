		...
		<td style="text-align: left">
			<?php
				...
				if (Kohana::config('config.pnp4nagios_path')!==false)
					echo (pnp::has_graph($host->host_name))  ? '<a href="' . url::site() . pnp::controller_path($host->host_name) . '" style="border: 0px">'.html::image($this->add_path('icons/16x16/pnp.png'), array('alt' => $this->translate->_('Show performance graph'), 'title' => $this->translate->_('Show performance graph'), 'class' => 'pnp_graph_icon')).'</a> &nbsp;' : '';
			...
			<?php
				...
				if (Kohana::config('config.pnp4nagios_path')!==false) {
					if (pnp::has_graph($service->host_name, urlencode($service->service_description)))
						echo '<a href="' . url::site() . pnp::controller_path($service->host_name, $service->service_description).'" style="border: 0px">'.html::image($this->add_path('icons/16x16/pnp.png'), array('alt' => $this->translate->_('Show performance graph'), 'title' => $this->translate->_('Show performance graph'), 'class' => 'pnp_graph_icon')).'</a> &nbsp;';
				}
